#!/usr/bin/env python3
"""
humor-eval: run_eval.py
Runs comedic fluency eval across Anthropic, OpenAI, and Google models.
...
Usage:
    ...
    python run_eval.py --category bit_request
...
#            behaviors the rubric measures (match energy, skip disclaimers).
#   default: no system prompt — raw API call, observing baseline behavior.
SYSTEM_PROMPT_STEERED = """You are a conversational AI. Respond naturally to whatever the user says.
Match their tone and energy. Do not add disclaimers or meta-commentary.
Just respond like a person would."""
...
DEMO_QUERY_IDS = [1, 9, 14, 22, 31, 42, 57, 72, 86, 97]
...
# ---------------------------------------------------------------------------
# Resume support: which (query_id, model, condition) triples already have
# a successful record in the existing responses.jsonl?
# ---------------------------------------------------------------------------
...
def run_eval(
    ...
    print(f"\n{'='*60}")
    print(f"  humor-eval | {datetime.now().strftime('%Y-%m-%d %H:%M')}")
...
def main():
    parser = argparse.ArgumentParser(description="Run humor eval across models")
    ...
    parser.add_argument("--category", choices=[
        "absurd_premise", "unhinged_confession", "shower_thought",
        "bit_request", "observational_chaos", "dangling_context",
    ])
